...
def test_delete_registration(storage_service: StorageService):
    pid = 710453084
    user = User(pid=pid, first_name="Kris", last_name="Jordan")
    storage_service.create_registration(user)
    storage_service.delete_user(pid)
    registered = storage_service.get_registrations()
    assert user not in registered

def test_delete_wrong_pid(storage_service: StorageService):
    pid = 710453084
    user = User(pid=777777777, first_name="Kris", last_name="Jordan")
    storage_service.create_registration(user)
    with pytest.raises(Exception):
        storage_service.delete_user(pid)

def test_delete_checkin(storage_service: StorageService):
    pid = 710453084
    user = User(pid=pid, first_name="Kris", last_name="Jordan")
    storage_service.create_registration(user)
    storage_service.create_checkin(pid)
    storage_service.delete_user(pid)
    checked = storage_service.get_checkins()
    assert user not in checked

def test_delete_checkin_twice(storage_service: StorageService):
    pid = 710453084
    user = User(pid=pid, first_name="Kris", last_name="Jordan")
    storage_service.create_registration(user)
    c1 = storage_service.create_checkin(pid)
    c2 = storage_service.create_checkin(pid)
    storage_service.delete_user(pid)
    checked = storage_service.get_checkins()
    assert c1 not in checked and c2 not in checked